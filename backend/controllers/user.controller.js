...
const convertUserDataTOPDF = async (userData)=>{
   ...
   const shiftLeft = 205; // Amount to shift the image to the left

   const x = (doc.page.width - 50) / 2 - shiftLeft; // Adjust x to move left

   const y = 10; // Keep the vertical position the same

   doc.image(`uploads/${userData.userId.profilePicture}`, x, y, { width: 50 });
...
export const whatAreMyConnections = async (req,res) =>{
    ...
    try{
        ...
        const connnections = await ConnectionRequest.find({connectionId: user._id})
        .populate('userId', 'name username email profilePicture');

        return res.json(connnections);
